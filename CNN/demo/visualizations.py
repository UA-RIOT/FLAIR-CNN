"""
CNN/demo/visualizations.py

Plotly chart helpers for the FLAIR-CNN Streamlit demo.
Each function accepts numpy arrays and returns a go.Figure.
"""
...
def input_heatmap(x_num: np.ndarray, feature_names: list) -> go.Figure:
    ...
    Rows = 10 flows (timesteps), cols = 21 numeric features.
    Color scale centered at 0 (data is z-score normalized).
    """
...
def reconstruction_comparison(
    ...
    Three-panel figure:
      ...
      Top-right: CNN-reconstructed heatmap
      Bottom:    per-feature reconstruction error (MSE) bar chart
    """
    flow_labels = [f"Flow {i}" for i in range(x_num.shape[0])]

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=("Original", "CNN Reconstruction", "Per-Feature Reconstruction Error (MSE)", ""),
        specs=[
    ...
    # Per-feature MSE bars — color intensity proportional to error magnitude
    ...
    fig.update_layout(
        title=dict(text="CNN Decoder Reconstruction vs Original", font=dict(size=14)),
        height=520,
...
def anomaly_gauge(score: float, threshold: float) -> go.Figure:
    """
    Gauge chart: green zone = normal, red zone = attack.
    Needle points to current score; vertical line marks threshold.
    """
